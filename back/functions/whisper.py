...
def download_audio(file_url, input_path):
...
@https_fn.on_request()
def whisper(req: https_fn.Request) -> https_fn.Response:
    """음성 파일을 변환하고 Storage 와 Firestore에 저장"""
    try:
        # Firestore에서 `voice` URL 가져오기
        doc_ref = db.collection('consult').document("user")
        doc = doc_ref.get()
        if not doc.exists:
        ...
        # 음성 파일 다운로드
        if not download_audio(file_url, input_path):
        ...
        transcript_url = upload(output_path, storage_file_path)
